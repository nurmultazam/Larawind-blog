...
@section('content')
    ...
    <div class="bg-gray-100 flex-1 p-6 md:mt-16">
        <h1 class="flex mb-3 text-2xl font-bold text-slate-700">My Posts</h1>
        <hr>

        <a href="/dashboard/posts/create" class="btn my-4 px-4 py-2 max-w-xs">Create new post</a>
        <!-- Start Table Sales -->
                ...
                <tbody class="text-gray-700">
                    @foreach ($posts as $post)
                        <tr>
                            ...
                            <td class="border border-l-0 px-4 py-2">{{ $post->category->name }}</td>
                            <td class="border border-l-0 border-r-0 px-4 py-2">
                                <div class="flex space-x-1 justify-center">
                                    <a href="/dashboard/posts/{{ $post->slug }}" class="btn-bs-primary text-sm px-2 py-1">
